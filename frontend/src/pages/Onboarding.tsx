...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, Swords } from 'lucide-react'
import { api } from '../services/api'
import { supabase } from '../lib/supabase'
import type { StudentProfile } from '../types'
import type { AssessmentQuestion, AssessmentResult } from '../types/assessment'

const interests = ['Technology', 'Design', 'Business', 'Finance', 'Science', 'Healthcare', 'Law', 'Media', 'Education', 'Social Sciences']
...
export default function Onboarding({ onCreated }: { onCreated: (profile: StudentProfile) => void }) {
  ...
  const [exploredStreams, setExploredStreams] = useState<Record<string, boolean>>({})
  ...
  const toggle = (key: 'subjects'|'interests'|'strengths'|'work_styles', value: string) => setForm(f => {
    if (key === 'interests' && !f[key].includes(value) && f[key].length >= 3) return f
    return { ...f, [key]: f[key].includes(value) ? f[key].filter(x => x !== value) : [...f[key], value] }
  })

  const valid = useMemo(() => {
    if (step === 1) return form.name.trim() && form.city.trim()
    if (step === 2) return form.subjects.length >= 2
    if (step === 3) return form.interests.length === 3 && form.strengths.length >= 2
  ...
  if (step === 5) {
    ...
    return <div className="onboarding assessment-page">
      <header className="landing-nav"><button className="text-btn" onClick={() => setStep(4)} disabled={saving}><ArrowLeft size={16} /> Back</button><div className="brand"><div className="brand-mark"><Swords size={18} /></div><strong>SkillSensei</strong></div><span className="step-counter">{stepLabel}</span></header>
  ...
  if (step === 6 && assessmentResult) {
    return <div className="onboarding assessment-page">
      <header className="landing-nav"><div className="brand"><div className="brand-mark"><Swords size={18} /></div><strong>SkillSensei</strong></div><span className="step-counter">{stepLabel}</span></header>
      <div className="onboarding-wrap"><div className="assessment-result-wrap">
        ...
        <div className="result-grid"><div className="result-card"><div className="result-card-head"><div><span className="eyebrow">Your stream suggestions</span><h2>What your profile suggests now</h2></div><span className={`alignment-pill ${assessmentResult.status}`}>{assessmentResult.status === 'strong_match' ? 'Strong alignment' : assessmentResult.status === 'partial_match' ? 'Partial alignment' : 'Broadened view'}</span></div><div className="stream-suggestion-list">{assessmentResult.stream_suggestions.map((item, index) => {
          const key = `${item.recommendation_id || item.stream_id}-${index}`
          const isExplored = exploredStreams[key]
          return <div className="stream-suggestion" key={key}><div><strong>{item.stream}</strong>{item.source === 'assessment' && <span className="assessment-tag">Assessment pattern</span>}</div><div className="stream-subjects">{(item.focus_subjects || []).map(subject => <span key={subject}>{subject}</span>)}</div>
          {item.explore_subjects && item.explore_subjects.length > 0 && !isExplored && <button className="text-btn" style={{marginTop: '8px', marginBottom: '8px', fontSize: '13px', fontWeight: 600, color: 'var(--accent)'}} onClick={() => setExploredStreams(prev => ({...prev, [key]: true}))}>Explore new opportunities</button>}
          {isExplored && <div className="stream-subjects explored-subjects" style={{marginTop: '8px', marginBottom: '8px'}}><div style={{fontSize: '12px', opacity: 0.7, marginBottom: '4px', width: '100%'}}>Aligned with your interests:</div>{(item.explore_subjects || []).map(subject => <span key={subject} style={{borderStyle: 'dashed'}}>{subject}</span>)}</div>}
          <span>{item.source === 'assessment' ? 'Suggested from how you answered the assessment.' : 'Suggested from the interests you entered.'}</span></div>
        })}</div></div>
  ...
  return <div className="onboarding"><header className="landing-nav"><button className="text-btn" onClick={() => setStep(s => Math.max(1, s - 1))} disabled={saving}><ArrowLeft size={16} /> Back</button><div className="brand"><div className="brand-mark"><Swords size={18} /></div><strong>SkillSensei</strong></div><span className="step-counter">{stepLabel}</span></header><div className="onboarding-wrap"><div className="progress-line"><span style={{ width: `${step * 20}%` }} /></div><div className="form-card">
    ...
    {step === 3 && <><span className="eyebrow">Your signals</span><h1>What pulls your attention?</h1><p>Choose exactly 3 interests that you naturally enjoy, and pick the abilities you trust yourself in.</p><h4>Interests (Pick exactly 3)</h4><div className="chip-grid small">{interests.map(x => <button key={x} className={`choice-chip ${form.interests.includes(x) ? 'selected' : ''}`} onClick={() => toggle('interests', x)}>{form.interests.includes(x) && <Check size={14} />}{x}</button>)}</div><h4>Strengths</h4><div className="chip-grid small">{strengths.map(x => <button key={x} className={`choice-chip ${form.strengths.includes(x) ? 'selected' : ''}`} onClick={() => toggle('strengths', x)}>{form.strengths.includes(x) && <Check size={14} />}{x}</button>)}</div></>}
